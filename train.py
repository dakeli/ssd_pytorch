...
import argparse
import os
from plot_kernel import plot_kernels
...
parser.add_argument('--dataset', default='VOC_300', choices=['VOC_300', 'COCO', 'VOC_512'], type=str, help='VOC or COCO')
...
if torch.cuda.is_available():
    ...
    if not args.cuda:
        print("WARNING: It looks like you have a CUDA device, but aren't " + "using CUDA. Run with --cuda for optimal training speed.")
...
def train():
    ...
    if args.cuda:
        net = torch.nn.DataParallel(ssd_net)     # by li
    ...
    # epoch_size = -10 * (len(dataset) // (-args.batch_size))
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        # base_net = [i for i in ssd_net.children()][0]
        # layer1 = base_net[0]
        # plot_kernels(layer1.weight.data.cpu().numpy())
        #
        #
        # viz.images(layer1.weight.data[0, :, :, :])
        ...
        if args.visdom:
            update_vis_plot(iteration, loss_l.detach(), loss_c.detach(), iter_plot, epoch_plot, 'append')

        if iteration != 0 and iteration % 10000 == 0:
            print('Saving state, iter:', iteration)
            torch.save(ssd_net.state_dict(), 'weights/' + args.dataset + '_' + repr(iteration) + '.pth')
