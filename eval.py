...
def test_net(save_folder, net, cuda, show_results, dataset, transform, top_k, im_size=300, thresh=0.05):
        ...
        if show_results:
            ...
            for k in range(cls_dets.shape[0]):
                ...
                if x1-x0 <= 96 and y1-y0 <= 96:
                    continue
                cv2.rectangle(img, (x0, y0), (x1, y1), color[m], 5)
            cv2.imwrite(VOC_ROOT+str(dataset.ids[i]).split(r'\\')[-1].split('\'')[-2] + '.png', img)
...
if __name__ == '__main__':
    ...
    test_net(args.save_folder, net, args.cuda, args.show_results, dataset,BaseTransform(net.size, dataset_mean), args.top_k, 300,
